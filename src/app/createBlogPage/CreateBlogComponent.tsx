// // app/createBlogPage/CreateBlogComponent.tsx
// "use client";
...
export default function CreateBlogComponent() {
  ...
  const createBlogMutation = useMutation({
    ...
    onError: (error: AxiosError) => {
      ...
      toast({
        title: "Error",
        description: axiosError.response?.data?.message || "Failed to create blog. Please try again.",
        variant: "destructive",
        className: "text-white bg-blue-950 hover:bg-black",
      })
    },
  ...
  return (
    <div className="min-h-screen w-full fixed inset-0 bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center p-4 sm:p-6 lg:p-8 overflow-y-auto">
      <Card className="w-full max-w-3xl bg-gray-800 border-gray-700 shadow-lg">
        <CardHeader className="space-y-1">
          <CardTitle className="text-3xl font-bold text-gray-100">Create a New Blog Post</CardTitle>
          <CardDescription className="text-gray-400 text-lg">Share your thoughts with the world</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-200 text-lg">Title</FormLabel>
                    <FormControl>
                      <Input
                        placeholder="Enter the title of your blog post"
                        {...field}
                        className="bg-gray-700 text-gray-100 border-gray-600 focus:border-primary focus:ring-primary text-lg py-2"
                      />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-200 text-lg">Content</FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder="Write your blog post here..."
                        {...field}
                        className="bg-gray-700 text-gray-100 border-gray-600 focus:border-primary focus:ring-primary min-h-[200px] text-lg py-2"
                      />
                    </FormControl>
                    <FormMessage className="text-red-400 italic" />
                  </FormItem>
              ...
              <div className="space-y-4">
                ...
                >
                  <LuImagePlus className="mr-2" />
                  {showDropBox ? "Hide Image Upload" : "Add Image"}
                </Button>
                {showDropBox && (
                  <div className="border-2 border-dashed rounded-md border-gray-700 p-4 transition-all duration-300 ease-in-out">
                    <ImageDropzone
                      onFileSelect={(file) => {
                        setUploadedImage(file)
                        setPreviewUrl(URL.createObjectURL(file))
